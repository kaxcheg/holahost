# lead-capture — техническая спецификация микросервиса

> Микросервис **lead-capture** в составе приложения **Holahost** (менеджмент туристической
> недвижимости: календарь, цены, коммуникация с гостями). Реализует публичное lead-magnet демо —
> guest-message reply assistant (RAG по STR-гайдбуку). Микросервис намеренно владеет доменом
> **Lead + Guidebook + Chunk** (+ rate-limit, sample-budget); фронтенд — часть Holahost и потребляет
> API/ассеты сервиса через границу владения.
> Целевая аудитория демо: индивидуальные / prosumer-хосты, 1–3 апартамента.
> Обзор большого приложения — `holahost/docs/holahost_overview.md`; карта владения — корневой `README.md`.
> Документ собирается поэтапно. Каждый этап — отдельный раздел верхнего уровня.
>
> **Соглашение о путях в этом документе:** `backend/…`, `infra/…`, `docs/…` — относительно корня
> сервиса (`holahost/services/lead-capture/…`); `frontend/…` — относительно приложения Holahost
> (`holahost/frontend/…`); «корень репо» — корень монорепозитория.
...
- Это SLO (CloudWatch alarm на 60s/8s по `p95(duration)` за 15 минут), а не enforced timeout. Hard timeout API Gateway — 30s по умолчанию (см. §2.1); потребуется поднять до 90s через Lambda Function URL (нет API Gateway): Lambda max execution time = 15 минут, Function URL timeout = тот же → hard timeout `= 90s` задаётся в Terraform на Function URL. **НЕ** `Settings`-поле: у приложения нет рантайм-консьюмера этого значения (AWS убивает Lambda сам).
...
Секреты dev в файлах не хранятся: `dev.env` — committed и non-secret; единственный реальный dev-секрет (`SAMPLE_SERVER_API_KEY`) передаётся в рантайме через `make dev-up`.
