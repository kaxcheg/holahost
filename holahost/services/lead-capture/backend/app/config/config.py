...
class Settings(BaseSettings):
    ...
    * ``SMTP_HOST`` / ``SMTP_PORT`` are dev-only, consumed DIRECTLY by the dev email branch
    ...
    frontend_origin: str
    # Mount path the app gateway serves this service at (`/api/<service>`, §11.6) — the SINGLE source of
    # the API prefix: the frontend calls it (VITE_API_BASE_URL), the router strips it from rawPath, and
    # the gateway CloudFront behavior routes it. Env-invariant; injected from <env>.env (dev via
    # docker-compose, staging/prod via Terraform).
    api_base_url: str
