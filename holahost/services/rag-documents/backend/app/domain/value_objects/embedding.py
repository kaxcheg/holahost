...
class Embedding:
    ...
    def __post_init__(self) -> None:
        # Embedder-computed, not client input — any violation here is an internal
        # defect, so `field` stays None throughout and nothing translates these to a 4xx.
        if len(self.value) != EMBEDDING_DIM:
            raise DomainValidationError(f"Embedding dimension must be {EMBEDDING_DIM}")
        if not all(math.isfinite(x) for x in self.value):
            raise DomainValidationError("Embedding values must all be finite")
        norm = math.sqrt(sum(x * x for x in self.value))
        if abs(norm - 1.0) > _NORM_TOLERANCE:
            raise DomainValidationError(f"Embedding must be L2-normalized (got norm={norm})")
