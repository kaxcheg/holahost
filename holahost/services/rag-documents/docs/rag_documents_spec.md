...
class RateLimiter(Protocol):
    ...
    # concurrency: check-and-increment должен быть атомарным (лок на bucket-ключ либо
    #              атомарный примитив) — иначе два конкурентных запроса читают одно
    #              pre-increment-значение и оба проходят, лимит протекает.
```
