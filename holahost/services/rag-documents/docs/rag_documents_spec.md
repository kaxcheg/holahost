...
Колонки «класс» нет, потому что идентичность **и есть** имя класса — колонка повторяла бы первую.
...
`type(self).__name__`). Не объявляется рядом с классом и не преобразуется. Идентичность ошибки и
форма её `details` — две половины одного контракта, и `details_dict()` уже живёт на классе;
отдельная строка `code` рядом растащила бы контракт по двум местам, которые надо держать равными
руками, и не купила бы ничего: на `code` нигде внутри не диспетчеризуют, статус берётся из таблицы
типов интерфейсного слоя.

Дословно, а не отображением в `ERR_SCREAMING_SNAKE`, как было до этого. Отображение не добавляло
информации и стоило единственного, что важно при чтении лога: значение из лога переставало
грепаться до класса, который его произвёл, — строки не существовало нигде в исходнике. Плюс ему
требовалась оговорка про подряд идущие заглавные (`HTTPError`). Переименование класса — изменение
контракта: оно даёт диф в `docs/openapi.json` и роняет `make openapi-check`, вместо того чтобы
пройти молчаливым рефакторингом.
...
**Общих кодов нет.** Раньше `UploadTooLargeError`/`ParsedTextTooLargeError` делили один код, а
`DocumentParseError`/`MalformedRequestError` — другой, на том основании, что код называет «факт для
клиента». Не называет: текст для пользователя вызыватель собирает из `details`, поэтому
идентичность обязана называть *какую ошибку бросил сервис*. Это были четыре разные ошибки,
различимые только копанием в значении `details` (`field == "file"`) либо статусом. Одна ошибка —
одна идентичность.
...
Контракт публикуется машиной, а не прозой: `interface/http/error_schemas.py` описывает те же
ошибки Pydantic-моделями, роуты объявляют их в `responses`, и всё уезжает в `docs/openapi.json`,
который CI диффит (`make openapi-check`). Две декларации одного контракта — осознанная цена
...
До этого схема была не просто неполной, а **неверной**: единственным ответом об ошибке в ней был
фастапишный `422 HTTPValidationError` с телом `{"detail": [...]}`, которого сервис не отдаёт
никогда — `handle_validation_error` перекрывает его на общий конверт.
...
Три ошибки в таблице поднимает и ими же сразу отвечает middleware, до маршрутизации:
...
На контракт это не влияет: в `openapi.json` они объявлены наравне с остальными, потому что
вызыватель может их получить.
...
Декоратора `wrap_value_error`, который раньше оборачивал голый `ValueError` в базовый
`ApplicationError` на каждом use case'е, больше нет. Он был неявным в обе стороны: его легко было
не поставить на новый use case, а поскольку `DomainValidationError` сам является `ValueError`, он
проглатывал и клиентскую половину — отдавая `500` там, где §7.6 требует `422`, и теряя `field`.
Его роль выполняет один явный обработчик `DomainValidationError` в интерфейсном слое.
...
op_completed { request_id, route, outcome, duration_ms,
               ...
               error_reason? }                                # только 5xx
```

Код отказа отдельным полем не пишется: им является сам `outcome` — для отказов это либо
идентичность ошибки (`NotFoundError`), либо строка HTTP-статуса (`"401"`), и именно по нему матчат
все фильтры отказов ниже.
`error_reason` — единственное поле со свободным текстом, и единственное, которое проходит через
скрабер токенов и адресов; оно заполняется только там, где тело ответа причины не несёт вовсе, то
есть на `5xx` (§7.6). Скрабит его сам `log_event` (`config/logging.py`), вызывающий передаёт
причину сырой — поэтому поле названо по тому, что в нём лежит, а не по обработке: прежнее
`error_message_sanitized` описывало обработку и притом со стороны вызывающего, который ничего
подобного не делал.

Уровень события — часть контракта, а не оформление: `ERROR` на `5xx`, `WARNING` на любой отказ,
вызванный вызывателем (включая те, что поднимает middleware до маршрутизации), `INFO` на успех и на
`startup_completed`. Фильтры CloudWatch матчат по `$.outcome` и уровня не читают — он существует для
человека и для любого правила «покажи ошибки» в просмотрщике логов. Порог логгера — `INFO`, так что
пишется всё.
...
| Частота отказов по лимиту | `outcome = RateLimitExceededError` | подтверждает адекватность `RATE_LIMIT_*` |
