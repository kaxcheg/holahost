...
| `absent` | create: MIME вне whitelist | `absent` | 415 `UnsupportedMediaTypeError` |
| `absent` | create: размер > лимита | `absent` | 413 `UploadTooLargeError` |
| `absent` | create: после парсинга текста нет | `absent` | 422 `EmptyDocumentError` |
| `absent` | create: чанков > лимита | `absent` | 422 `TooManyChunksError` |
...
| `absent` | search/get/delete по неизвестному id | `absent` | 404 `NotFoundError` |
...
Конкретные числа лимитов, порог сходства и K — Этап 3. Полный контракт ошибок и формат тела
ошибки — Этап 7.
...
| `code` | идентичность ошибки — имя класса исключения; контракт и тело — Этап 7 |
...
- Файл с MIME вне `ALLOWED_MIME_TYPES` → `415 UnsupportedMediaTypeError`; в сообщении перечислены поддерживаемые форматы.
- Файл больше `MAX_UPLOAD_SIZE` → `413 UploadTooLargeError`; в `details` — фактический лимит.
- Пустое имя или имя длиннее `MAX_DOCUMENT_NAME_LENGTH` → `422 InvalidPayloadError` с указанием поля.
- Извлечено меньше `MIN_EXTRACTED_TEXT_CHARS` → `422 EmptyDocumentError`.
- Текст после парсинга длиннее `MAX_PARSED_TEXT_LENGTH` → `422 ParsedTextTooLargeError` с указанием лимита.
- Получилось больше `MAX_CHUNKS_PER_DOCUMENT` чанков → `422 TooManyChunksError`; в `details` — лимит и фактическое число.
...
- Замена чужого или несуществующего документа → `404 NotFoundError`.
...
- Запрос длиннее `MAX_QUERY_LENGTH` → `422 InvalidPayloadError`.
- Пустой запрос → `422 InvalidPayloadError`.
- Поиск по чужому или несуществующему `document_id` → `404 NotFoundError`.
...
- Чужой или несуществующий документ → `404 NotFoundError`.
...
- Последующий поиск и `get` по этому `document_id` → `404 NotFoundError`.
...
**AC:**
- Любая операция (`get`, `search`, `replace`, `delete`) с документом, у которого `owner` ≠ `sub` вызывающего токена, → `404 NotFoundError`; `403` в этом случае не используется, существование чужого документа не раскрывается.
...
**AC:**
- Мидлварь `holahost-auth` (общая библиотека, ADR A-5) подключена ко всем роутам, кроме `GET /api/rag-documents/health`.
...
- `GET /api/rag-documents/health` отвечает без токена.
...
**AC:**
- Тело любой ошибки — `{ error: { code, message, details } }`; `code` — идентичность ошибки из контракта сервиса.
...
- Каждой ошибке в контракте сопоставлена retry-семантика (повторять / не повторять) — Этап 7.
...
**AC:**
- `GET /api/rag-documents/health` доступен без авторизации и отвечает `200`, когда сервис готов
  принимать запросы. Под базовым путём сервиса, как и все остальные роуты (рамочная спека):
  гейтвей путь не переписывает, поэтому голый `/health` снаружи недостижим и smoke-проверка
  деплоя до него не дойдёт.
...
Не всё в этом дереве принадлежит сервису. `config/logging.py` объявляет только собственные поля
события; `application/ports/{exceptions,uow}.py` реэкспортируют платформенные типы;
`infrastructure/db/` держит схему, репозиторий и один локальный `engine.py` (та же фабрика движка
плюс кодек `pgvector`), а UoW, перевод ошибок драйвера и привязка RLS приходят из `holahost-db`;
`interface/http/errors.py` — это таблица статусов, а не обработчик. Что где живёт — рамочная спека,
...
| Общие библиотеки платформы | `holahost-observability` (JSON-логгер и allowlist), `holahost-http` (конверт ошибок, мидлвари края и их сборка, обработчик исключений), `holahost-auth` (валидация JWT и её конфиг, читающий окружение сам), `holahost-db` (контракт сбоев хранилища, UoW, настройки Postgres, RLS, провижининг роли) — рамочная спека, «Переиспользуемые общие сущности» |
...
| Исходный файл | **не хранится** | живёт в пределах запроса и отбрасывается после ответа; тело загрузки при этом спулится в анонимный временный файл (§3.8, «Парсинг») |
...
| `EMBEDDING_MODEL` | `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` | многоязычная (гайдбуки не только на английском), 384 измерения, работает на CPU; полное имя обязательно — реестр моделей `fastembed` отклоняет короткое |
...
| Аспект | Решение |
|---|---|
| Аутентификация | JWT оффлайн через `holahost-auth` на всех роутах, кроме `GET /api/rag-documents/health`; фиксированный `alg` из конфига |
| Авторизация | owner-only по `sub`; чужой ресурс → `404`, существование не раскрывается |
| Роли БД | две, и это условие работоспособности изоляции: **bootstrap-суперюзер** контейнера БД (создаётся initdb, применяет миграции, владеет таблицами) и **роль приложения** (создаётся шагом провижининга деплоя как `NOSUPERUSER NOBYPASSRLS`, получает только DML-гранты, таблицами не владеет). Изоляция владельца держится исключительно на RLS, которую суперюзер обходит безусловно — если приложение подключается суперюзером, изоляции нет ни на одном роуте. Приложение отказывается стартовать, если его роль способна обойти RLS |
| Секреты (dev) | оба пароля БД (роли приложения и bootstrap-суперюзера) — из `.env` окружения dev; в git попадает только `.env.example` |
| Секреты (staging / prod) | AWS Secrets Manager, **два** секрета: `db-password` (роль приложения) и `db-superuser-password` (bootstrap-суперюзер контейнера БД). Создаются собственным terraform-root'ом сервиса (рамочная спека, «Инфраструктура и CI/CD микросервиса») со значениями, заполняемыми вручную один раз; в образ не запекаются и в env compose-файла не хранятся. Значения обязаны различаться — см. «Роли БД» ниже. Ротация `db-password` завершается **следующим выкатом**, а не рестартом: пароль роли меняет шаг провижининга, образ postgres задаёт его только при initdb |
...
| Парсинг | без вызовов внешних утилит; OCR и исполнение вложенного контента не выполняются. Исходник не сохраняется как артефакт (A-10): ни бакета, ни колонки, ни пути — после ответа он недостижим. Гарантии «плейнтекст не оказывается на диске» приложение не даёт и дать не может: тело загрузки Starlette спулит в анонимный временный файл (без имени в ФС, блоки освобождает ядро при закрытии дескриптора, в том числе при падении процесса), а то, что лежит в памяти процесса, уходит на диск через swap или core dump. Это набор свойств хоста — без swap, дампы выключены, том шифрован — и ставится там, где определяется инстанс |
...
Ограничение структурное, а не по соглашению: `Chunk` неизменяем и живёт только вместе со своим
документом (см. Lifecycle ниже).
...
-- приложение, а не что-либо в этой миграции. См. §8.0: окружение обязано развести
-- bootstrap-суперюзера контейнера БД и роль приложения, иначе эта политика не действует
-- нигде, кроме тестов.
...
| Что | Значение |
|---|---|
| Базовый путь | `/api/rag-documents` (сегмент = имя сервиса, рамочная спека). Объявлен один раз, выводится из имени сервиса; каждый роутер монтируется под него, включая health |
| Авторизация | `Authorization: Bearer <jwt>` на всех роутах, кроме `GET /api/rag-documents/health`; иных способов передачи секретов в контракте нет |
...
| `GET /api/rag-documents/health` | готовность | — | `200` |
...
```
GET /api/rag-documents/health          # без авторизации
...
Контракт фиксирует сервис: вот ошибки, которые он бросает. Каждая описывается двумя вещами и
только ими — **идентичностью** и **формой `details`**:

| Идентичность (`code`) | HTTP | Когда | `details` | Повторять? |
|---|---|---|---|---|
| `InvalidPayloadError` | 422 | пустое или слишком длинное имя, пустой или слишком длинный запрос, отсутствующее поле | `field`, `limit` | нет |
| `DocumentParseError` | 422 | файл повреждён либо не разбирается | — | нет |
| `MalformedRequestError` | 422 | отсутствующий `X-Request-ID` | — | нет |
| `UnsupportedMediaTypeError` | 415 | MIME вне `ALLOWED_MIME_TYPES` | `allowed` | нет |
| `UploadTooLargeError` | 413 | файл больше `MAX_UPLOAD_SIZE` — проверка **до** парсинга | `limit`, `actual` | нет |
| `ParsedTextTooLargeError` | 422 | текст после парсинга длиннее `MAX_PARSED_TEXT_LENGTH` | `limit`, `actual` | нет |
| `EmptyDocumentError` | 422 | извлечено меньше `MIN_EXTRACTED_TEXT_CHARS` | `min_chars` | нет |
| `TooManyChunksError` | 422 | чанков больше `MAX_CHUNKS_PER_DOCUMENT` | `limit`, `actual` | нет |
| `NotFoundError` | 404 | документа нет либо он принадлежит другому субъекту | — | нет |
| `InternalError` | 500 | всё, чего в этой таблице нет | — | да, однократно |

Отдельно — **платформенные ответы**. Их производит общий edge, они одинаковы за любым сервисом
платформы и в контракт этого сервиса не входят:

| Ответ | Кто | Когда |
|---|---|---|
| `401` `{"detail": "Unauthorized"}` | `holahost-auth` | токена нет, он испорчен либо отвергнут |
| `503` `{"detail": "Service Unavailable"}` | `holahost-auth` | JWKS недоступен — не проблема учёток |
| `429 RateLimitExceededError` (`retry_after_seconds`) | `holahost-http` | превышен per-caller лимит; `Retry-After` обязателен |
| `413 PayloadTooLargeError` (`limit`, `actual`) | `holahost-http` | тело запроса больше транспортного капа — отказ до чтения |

`docs/openapi.json` их не описывает намеренно: повторять один и тот же факт в документе каждого
сервиса значит превращать его в N фактов, которые разъедутся. `MalformedRequestError` — не
исключение из этого правила, а его подтверждение: доставляет его платформенный
`RequestIdMiddleware`, но сама ошибка сервиса (идентичность и немой `details` объявлены в
`application/exceptions`), поэтому она в таблице выше и в схеме.

Три идентичности из таблицы — `InvalidPayloadError`, `MalformedRequestError` и `NotFoundError` —
объявлены не здесь, а в `holahost-http`: их отвечает каждый сервис платформы, с той же формой
`details`. `application/exceptions` их реэкспортирует, поэтому сервис по-прежнему называет все свои
ошибки в одном месте, а `ERROR_CONTRACT` сопоставляет им статусы наравне с собственными.

**Идентичность — это имя класса, дословно** (`PlatformError.code` возвращает
`type(self).__name__`). Не объявляется рядом с классом и не преобразуется: идентичность и форма
`details` — две половины одного контракта, а `details_dict()` уже живёт на классе, так что
отдельная строка `code` растащила бы контракт по двум местам, равным только вручную. Значение из
лога грепается до класса, который его произвёл, а переименование класса — изменение контракта: оно
даёт диф в `docs/openapi.json` и роняет `make openapi-check`.

Единственная идентичность без класса за ней — `InternalError`. Класса нет намеренно: заводить
никогда не бросаемый класс значило бы намекать, что его кто-то бросает, тогда как смысл ровно
обратный — это ответ «не одна из перечисленных выше».

**Общих кодов нет: одна ошибка — одна идентичность.** Текст для пользователя вызыватель собирает
из `details`, поэтому идентичность обязана называть, *какую ошибку бросил сервис*, а не категорию,
в которую та попадает.

**В контракт не входят:**

- **текст (`message`)** — он для человека, читающего лог или ответ руками. Вызыватель его не парсит
  и пользователю не показывает: то, что видит пользователь, собирается из `code` и `details`.
  Поэтому он и **не публикуется**: на проводе он есть, в `docs/openapi.json` его нет ни у одной
  ошибки. Объявить его в схеме значило бы пригласить ровно тот разбор, который этот пункт
  запрещает;
- **HTTP-статус** — проекция, которую накладывает интерфейсный слой (`ERROR_CONTRACT` в
  `interface/http/errors.py`). Поэтому две ошибки могут отвечать одним статусом, и поэтому смена
  статуса — не изменение этого раздела.

`details` — размеченное объединение, а идентичность его тег: форма зависит от того, какая ошибка
брошена, и прочитать `details.limit` можно, только зная, что у этой ошибки такой ключ есть. Отсюда
правило: **у одной идентичности всегда один и тот же набор ключей**, `null` вместо отсутствующего
ключа (US-R09). Так `InvalidPayloadError` всегда отдаёт и `field`, и `limit`, где `limit = null`
означает «у этого поля нет ограничения длины».

Контракт публикуется машиной, а не прозой: `interface/http/error_schemas.py` описывает
собственные ошибки сервиса Pydantic-моделями (платформенные приходят готовыми из
`holahost_http.error_schemas`), роуты объявляют их в `responses`, и всё уезжает в
`docs/openapi.json`, который CI диффит (`make openapi-check`). Две декларации одного контракта — осознанная цена
слоистости (application-слой не должен зависеть от сериализации), и их расхождение ловит
`tests/unit/interface/http/test_error_schemas.py`.

Генератор FastAPI читает **только сигнатуру роута** — `response_model`, `status_code`,
`responses=`, параметры и зависимости; ни обработчики исключений, ни middleware ему не видны.
Поэтому то, что вызыватель обязан прислать сам, объявлено там же: bearer-токен через
`HTTPBearer(auto_error=False)`. Зависимость ничего не проверяет: edge ответил задолго до того, как
дойдёт до неё очередь, и дублирующий гейт стоял бы за первым, в другом порядке и со своим телом.

`X-Request-ID` обязателен не менее строго и **не публикуется**. Он не вызывателя: оба штатных входа
проставляют его безусловно (§3.1) — этим он и отличается от токена, который вызыватель получает и
предъявляет сам. И публикация сняла бы немоту `MalformedRequestError`: та ошибка молчит о том,
какого заголовка не хватило, именно потому, что запрос без него пришёл путём, которым не должен был
идти, — а схема, называющая заголовок, выдаёт такому вызывателю единственную нужную ему подсказку.

**Что публикуется, решает членство в `ERROR_CONTRACT`.** Класса там нет — значит это не ошибка
сервиса, чем бы она ни была: неучтённый подкласс `ApplicationError`, `DomainValidationError`,
`KeyError` из чужой библиотеки. Все они получают `500 InternalError` с пустым телом; причина уходит
в лог. Так словарь остаётся замкнутым, хотя пространство исключений Python открыто. Подкласс
опубликованной ошибки отвечает идентичностью **предка**, а не своей: своей нет в схеме.

`InvalidPayloadError` — `422`, не `400`: по RFC 9110 §15.5.1 `400` покрывает испорченный
синтаксис/фрейминг запроса, а по RFC 4918 §11.2 (происхождение `422`) этот код обозначает
синтаксически корректный запрос, содержимое которого не удалось обработать. Совпадает с поведением
FastAPI по умолчанию для `RequestValidationError`. `400` в этом сервисе не используется ни одним
мэппингом — зарезервирован за нарушением HTTP-фрейминга, которое сервис сейчас не производит.

Три ошибки поднимает и ими же сразу отвечает middleware, до маршрутизации:
`MalformedRequestError`, `PayloadTooLargeError`, `RateLimitExceededError`. Их нет в
`ERROR_CONTRACT` — статус каждой задан аргументом там, где собирается стек
(`interface/http/app.py`), и строка в таблице обработчика была бы вторым источником того же числа.
Механика обработчика при этом платформенная (`holahost_http.register_error_handlers`); этому сервису
принадлежат ровно три вещи, и они лежат в `interface/http/errors.py`: сама таблица, логгер, куда
уходят отказы, и список типов, отвечающих `500` молча (здесь — `DomainValidationError`).
В `openapi.json` описан только `MalformedRequestError`: две остальные — платформенные ответы
(таблица выше), и документ сервиса их не повторяет.

`401` и `503` кода не имеют вовсе — они отвечают `{"detail": "..."}` и принадлежат общей
библиотеке авторизации. `401` не раскрывает причину (не помогаем перебору), она уходит в лог.
`503` — недоступный JWKS: отказ каждому валидному токену, и отдавать его как `401` значило бы
выдать инцидент за волну плохих учёток. `403` этот сервис не производит: чужой ресурс — `404`
(A-13), то есть пути «токен валиден, прав нет» здесь нет.
...
class DocumentsRepo(ABC):
    ...
    # репо дублирует то, что уже даёт RLS.)
...
# VectorSearch — read-side Chunk'а (DocumentsRepo — write-side, та же БД): косинусное
# сходство считает pgvector на стороне БД (A-2), поэтому порт принимает document_id, а не
# список чанков. Свой порт, а не метод DocumentsRepo: возвращает узкую проекцию
# (SearchHit) и никогда не реконструирует сущность.
...
# holahost_http (LIB-02). Мидлварь вызывает лимитер до входа в юзкейс (§8.1 шаг 3), поэтому
# ни один порт application-слоя его не видит и объявлять здесь нечего. Сервис объявляет
# только значения: свои вёдра (bucket_for) и потолки по (bucket, is_service).
#
# Сигнатура платформенного порта — keyword-only и с is_service:
#     check(*, client_id: str, subject: str, bucket: str, is_service: bool) -> None
#     raises: RateLimitExceededError (несёт retry_after)
#     concurrency: check-and-increment должен быть атомарным — иначе два конкурентных
#                  запроса читают одно pre-increment-значение и оба проходят, лимит протекает.
# is_service передаётся, а не выводится из subject == client_id внутри лимитера: это факт о
# модели токена, и у него уже есть владелец — holahost-auth (LIB-01).
# Счётчики остаются process-local (A-8): библиотекой стала механика, а не хранилище счётчиков.
```
...
обязан объявленное обработать либо осознанно пропустить наверх. Три типа сбоев хранилища и перевод
вендорских ошибок драйвера в них — платформенные, из `holahost-db`: `application/ports/exceptions.py`
их реэкспортирует и добавляет только свой `EmbeddingFailedError`. Так же и `UnitOfWork` — Protocol
платформенный, реэкспортируется в `application/ports/uow.py`. Методы, работающие с разделяемым состоянием, объявляют и контракт
...
идти внутри явного `with uow:`; «облегчённого» пути без транзакции нет ни у одного метода.

Классы исключений application-слоя несут только `details_dict()` — ни HTTP-статуса, ни объявленного
`code`. Идентичность — имя самого класса (§7.6), а статус — деталь HTTP-протокола и принадлежит
обработчику ошибок интерфейсного слоя, который сопоставляет статус **типу** исключения
(`ERROR_CONTRACT`, идиома `@app.exception_handler(SpecificType)`). Поэтому две ошибки могут отвечать
одним статусом, а смена статуса не трогает ни одного класса.
...
Далее — контур, одинаковый для всех роутов, кроме `GET /api/rag-documents/health`:

1. тот же `holahost_http.RequestIdMiddleware` — для этих роутов уже не только читает заголовок, но и
   требует, чтобы `X-Request-ID` присутствовал (какой ошибкой отвечать на его отсутствие, библиотека
   принимает аргументом — `interface/http/edge.py::MISSING_REQUEST_ID_ERROR`);
   ...
   трактуется как нарушение контракта: `422 InvalidPayloadError` **с пустым `details`** — единственный
   `InvalidPayloadError`, отдаваемый до аутентификации, поэтому причина уходит в лог, а не в тело
...
3. `RateLimiter.check(client_id=…, subject=…, bucket=…, is_service=…)` — `429 + Retry-After` при превышении; bucket = `"ingest"` для create/replace, `"read"` для search/get/delete — деление по операции, не по HTTP-методу (`POST /{id}/search` — `"read"`). Потолок задаётся парой `(bucket, is_service)`: у сервисного токена счётчик один на всю интеграцию, у обменянного — на пользователя.
...
| `EmbeddingFailedError` | **никто по пути** — осознанный пропуск | наружу `500 InternalError`; сбой модели не восстановим повтором в рамках запроса и не должен маскироваться |
...
| `StorageUnavailableError` | осознанный пропуск до обработчика | наружу `500 InternalError`; повтор внутри запроса бессмыслен |
| `IntegrityError` | осознанный пропуск до обработчика | наружу `500 InternalError`; это дефект — инварианты сущностей или блокировка должны были снять конфликт раньше |
...
| `DomainValidationError` с заданным `field` (VO/сущность, нарушение инварианта данными вызывателя) | use case, на той самой конструкции, которая может его бросить | транслирует в свою ошибку §7.6 (`InvalidPayloadError`, `UnsupportedMediaTypeError`), беря `field` из исключения (US-R01) |
| `ChunkCountExceededError` (подкласс, чанков больше `MAX_CHUNKS_PER_DOCUMENT`) | use case | транслирует в `TooManyChunksError` — наружу `422 TooManyChunksError` |
| `DomainValidationError` без `field` (VO-инварианты, вкл. `TextFragment`/`SimilarityScore` — application-слой, §4.1) | осознанный пропуск до обработчика | наружу `500 InternalError`, причина — только в лог; это внутренний дефект, ничем в запросе не вызванный, и транслировать его в `4xx` нечем |

Клиентскую половину от внутренней отличает `field`; отдельного типа для внутреннего дефекта нет
намеренно — вызывающий код на него никак не реагирует, он просто летит наверх. А вот
`ChunkCountExceededError` — тип, и ровно по тому же правилу: `Document.create` бросает
`DomainValidationError` на трёх инвариантах, из которых клиентский только один, и use case обязан
выбрать именно его, пропустив два остальных в `500`. Выбор строкой (`field == "chunk_count"`)
не виден тайпчекеру и тихо перестаёт совпадать при переименовании поля — с этого момента внутренний
дефект начинает отдаваться как ошибка вызывателя. Там, где `try` оборачивает одну конструкцию,
способную упасть только по-своему (`DocumentName`, `MimeType`), выбирать не из чего — там базового
типа с `field` достаточно, и подкласс не заводится.

Нетранслированный `DomainValidationError` ловит один явный обработчик в интерфейсном слое —
декоратора поверх каждого use case'а нет: он был бы неявным в обе стороны (легко не поставить на
новый use case, и, поскольку `DomainValidationError` сам `ValueError`, он проглотил бы и
клиентскую половину, отдавая `500` там, где §7.6 требует `422`).

### 8.7 Событие завершения операции

Метрика — структурное лог-событие; агрегация задаётся на стороне сбора логов (Этап 11). Имя
события, ядро его полей (`request_id`, `client_id`, `sub`, `route`, `outcome`, `duration_ms`,
`error_reason`) и уровни — **платформенный контракт**, см. рамочную спеку, «Контракт события
завершения операции». Здесь — только то, что этот сервис добавляет сверх ядра; сами метрики,
технические и продуктовые, собраны в разделе «Метрики» после беклога.

```
op_completed { <ядро платформы>,
               document_id?,
               stage_ms: { parse, chunk, embed, persist }?,   # только ingest
               chunk_count?,                                  # только ingest
               hits?, top_score? }                            # только search
```

Собственные поля сервиса объявлены списком в `config/logging.py` — это `document_id`,
`chunk_count`, `hits`, `top_score`, `stage_ms`; там же они передаются в `configure_logging`. Ядро
приходит из `holahost-observability` и здесь не повторяется; отказы до маршрутизации пишет
`holahost_http.log_rejection`, поэтому событие есть и у запроса, который не дошёл до обработчика.

**`route` приходит в двух разных форматах, и это несущий факт для всех фильтров.** На успехе
роутер пишет *шаблон* без префикса — `POST /documents`, `PUT /documents/{id}`,
`POST /documents/{id}/search`, `GET /documents/{id}`, `DELETE /documents/{id}`. На отказе
платформенный обработчик пишет `f"{method} {request.url.path}"` — **разрешённый** путь, с префиксом
`/api/rag-documents` и настоящим `document_id`. Префикса на успешном пути не бывает никогда,
поэтому одно его наличие надёжно отделяет отказ от успеха, не читая `outcome`.

`outcome` — либо `success`, либо идентичность ошибки (имя класса), либо голый числовой статус.
Общего префикса у идентичностей нет, поэтому «все 4xx» одним фильтром не выражается — доля отказов
считается через формат `route`, а конкретные отказы перечисляются поимённо.
...
| **Секреты в SM** (`infra/envs/<env>`) | состав секретов — в §3.8, строка «Секреты (staging / prod)»; здесь не дублируется. Внешних провайдеров у сервиса нет, ключей к ним в SM не появляется |
...
| **Алармы** (`infra/envs/<env>`) | аларм на `5xx`, фильтры отказов авторизации и лимита приходят из платформенного модуля `service-observability`. Сверх него — p95 длительности `POST /documents` (именно ingest первым упирается в потолок интеграции, §3.7), p95 поиска, доли успеха и маршрутные метрики §8.7: им нужно знать роут, поэтому они живут в роуте сервиса |
...
| **Smoke после выката** | `GET /api/rag-documents/health` отвечает `200` только после загрузки модели в память, а она на холодном volume скачивается. Smoke обязан ретраить до готовности с ограниченным окном ожидания, иначе выкат будет ложно падать |
| **Установка зависимостей в `ci`** | сверх умолчания: перед хуками ставятся все общие библиотеки, в порядке их path-зависимостей (`observability` → `http` → `auth`, `db` отдельно) — репо-корневые хуки итерируют по каждому пакету с `pyproject.toml`, и `poetry run` в пустом venv молча не находит инструмент |
...
Разработаны в объёме этой спеки и с этого момента обязательны для всех последующих сервисов
(рамочная спека, «Переиспользуемые общие сущности»).
...
- `LIB-02` `holahost-http` — конверт ошибок и `PlatformError`; мидлвари `X-Request-ID`, ограничителя тела и рейт-лимита; `RateLimiter` и его in-memory реализация; обработчик исключений по контракту сервиса; платформенные ошибки и их публикуемые схемы; объявление `bearerAuth`
- `LIB-03` `holahost-observability` — JSON-логгер, механизм allowlist полей, ядро полей `op_completed`, скрабер свободного текста
- `LIB-04` `holahost-db` — три типа сбоев хранилища и перевод вендорских ошибок, `UnitOfWork` и его SQLAlchemy-реализация, фабрика engine, повтор при конфликте, настройки двух Postgres-идентичностей, привязка RLS и стартовый guard, провижининг роли, скелет `alembic/env.py`
...
- `R-06` Application-исключения и их идентичности на проводе
...
- `R-13` Репозиторий документов (документ как агрегат над чанками, §4.3) на SQLAlchemy Core; политики Row-Level Security (§6) как механизм изоляции по владельцу. UoW, трансляция ошибок драйвера и привязка владельца — из `LIB-04`
...
- `R-18` Вёдра ingest/read, отображение роутов на них и потолки по видам идентичности; счётчик — `InMemoryRateLimiter` из `LIB-02`
- `R-19` Объявление собственных полей события (`document_id`, `chunk_count`, `hits`, `top_score`, `stage_ms`) поверх ядра из `LIB-03`; инструментовка `stage_ms` в интерфейсном слое

### Backend — Interface

- `R-20` FastAPI-приложение, роутер, схемы запросов и ответов
- `R-21` Мидлвари — `request_id`, авторизация через `LIB-01`, rate limit
- `R-22` `ERROR_CONTRACT` — таблица «ошибка → статус», логгер отказов и список типов, отвечающих `500` молча; механика — из `LIB-02`
- `R-23` `GET /api/rag-documents/health` с проверкой БД и готовности модели
...
- `R-26` TF-root `infra/common` — вызов платформенного модуля `service-ecr`
- `R-27` TF-root `infra/envs/<env>` — вызов платформенного модуля `service-observability` (группа логов, SNS-топик, аларм `5xx`), собственные value-less секреты, фильтры метрик раздела «Метрики», четыре аларма (p95 ingest, p95 поиска, две доли успеха) и дашборд, на котором видна **каждая** публикуемая метрика: фильтр без аларма и без виджета — метрика, которую никто не откроет
...
- `R-32` Makefile — `include ../../make/common.mk` плюс имя сервиса и потолок размера образа

---

## Метрики

Единственный источник — структурное лог-событие (A-14): ни агента метрик, ни Prometheus в топологии
...
значения или отношение метрик. Метрика, под которую в событии нет поля, здесь не появляется —
вместо неё стоит строка о том, чего не хватает. Форма события — §8.7; ядро полей и его имя —
платформенный контракт (рамочная спека).

Фильтры разложены по двум местам, и граница проходит по знанию о сервисе: то, что выводится из ядра
`op_completed` и ничего не знает про роуты, лежит в платформенном модуле `service-observability` и
написано один раз на всю платформу; всё, что матчит имя роута, идентичность своей ошибки или
доменное поле, лежит в `infra/envs/<env>/observability.tf` этого сервиса, рядом с именами, по
которым матчит.
...
мёртвую метрику как здоровье. Поэтому таблицы ниже называют поле и значение дословно.

### Технические метрики

Отвечают на вопрос «сервис исправен и укладывается в бюджеты?».
...
| p95 длительности ingest | `duration_ms` при `route = POST /documents` | сервис | да, `extended_statistic = p95` | контроль `INGESTION_P95_BUDGET` |
| p95 длительности поиска | `duration_ms` при `route = POST /documents/{id}/search` | сервис | да | контроль `SEARCH_P95_BUDGET` |
| Success rate ingest | metric math `success / (success + failure)` | сервис | да | ловит деградацию, которую ни один отдельный класс ошибок не показывает |
| Success rate поиска | то же по поисковым роутам | сервис | да | там же |
| Разбивка ingest по стадиям | `stage_ms.{parse,chunk,embed,persist}` | сервис | нет | показывает, во что упирается бюджет, до того как он превышен |
| Частота отказов авторизации | `outcome = 401` / `403` | модуль | нет | всплеск = вызыватель настроен неверно либо перебор |
| Частота отказов по лимиту | `outcome = RateLimitExceededError` | модуль | нет | подтверждает адекватность `RATE_LIMIT_*` |
| Частота отказов по размеру | `outcome = UploadTooLargeError` / `PayloadTooLargeError` | сервис | нет | различает «упёрся в прикладной лимит» и «упёрся в транспортный», то есть в разные числа §7.6 |
| Частота отказов по типу файла | `outcome = UnsupportedMediaTypeError` | сервис | нет | рост = вызыватель шлёт формат, которого нет в белом списке |
| Частота отказов разбора | `outcome = DocumentParseError` | сервис | нет | отделяет «файл битый» от «формат не поддержан»; рост при неизменном составе форматов = регресс парсера |
| Частота нарушений транспортного контракта | `outcome = MalformedRequestError` | сервис | нет | `X-Request-ID` проставляет каждый штатный путь входа, поэтому ненулевое значение = вызыватель идёт мимо гейтвея |
| Время прогрева процесса | `startup_completed.duration_ms` | сервис | нет | объясняет провал готовности после выката; smoke-проверка ждёт именно его |

Чего в этом списке нет и почему: **повторы при `ConcurrentUpdateError`** — `retry_on_concurrent_update`
повторяет транзакцию молча, и наружу выходит либо успех, либо отказ, поэтому число повторов ни в
одно поле не попадает; чтобы его считать, нужно отдельное событие, а не фильтр. **Задержка
доставки логов** — свойство сборщика, не сервиса.

### Продуктовые метрики

Отвечают на вопрос «сервисом пользуются, и он приносит пользу?». Читаются без знания кода, поэтому
формулировки здесь — на языке продукта, а не полей.

| Метрика | Источник | Зачем |
|---|---|---|
| Документов загружено за период | запрос `COUNT(*)` | рост корпуса; базовая мера принятия сервиса |
| Замен документов за период | `route = PUT /documents/{id}`, `outcome = success` | как часто контент обновляют, а не добавляют — разные сценарии хоста |
| Удалений за период | `route = DELETE /documents/{id}`, `outcome = success` | вместе с загрузками даёт прирост корпуса |
| Поисков за период | `route = POST /documents/{id}/search`, `outcome = success` | активность потребления; знаменатель почти для всего ниже |
| Поисков на документ | поиски ÷ документы | окупается ли загрузка: документ, который никто не ищет, стоил ingest'а зря |
| Доля результативных поисков | `hits > 0` ÷ все поиски | **главная продуктовая метрика качества**: доля запросов, на которые сервис вообще что-то ответил |
| Доля пустых поисков | `hits = 0` | она же с другой стороны, и прокси завышенного `SIMILARITY_THRESHOLD` |
| Распределение `top_score` | `top_score` | прямая база для калибровки порога замером, а не на глаз |
| Распределение числа чанков в документе | `chunk_count` | размер типичного документа в единицах, которыми оперирует сервис; заодно видно приближение к `MAX_CHUNKS_PER_DOCUMENT` |
| Уникальных владельцев за период | `sub` | сколько субъектов реально пользуются, а не сколько интеграций подключено |
| Уникальных вызывателей за период | `client_id` | то же со стороны интеграций |

Две последние строки и «поисков на документ» — **запросы, а не постоянные фильтры**: фильтр
CloudWatch превращает совпавшую строку в число и не умеет ни считать уникальные значения, ни делить
одну метрику на другую по ключу. Они считаются запросом Logs Insights (`stats count_distinct(sub)`)
по той же группе логов. Нового компонента это не вводит — группа та же, — но и постоянной метрики,
на которую можно повесить аларм, из них не получится; это цена событийной модели (A-14), а не
недоделка.
...
`pending` → `409 DocumentNotReadyError`. При замене прежняя версия остаётся доступной для поиска,
...
Decision: файл живёт в пределах запроса и отбрасывается; сервис хранит чанки и векторы, но не документы как файлы. «Не хранится» здесь означает «не сохраняется как артефакт» — нет ни бакета, ни колонки, ни пути; это утверждение про сервис, а не про то, касались ли байты диска (см. §3.8, «Парсинг»).
...
**A-13. Чужой ресурс — `404`, а не `403`**
...
**A-14. Метрики — структурные лог-события, без отдельной системы метрик**
...
**A-15. Модель эмбеддингов — многоязычная MiniLM-L12, 384 измерения**
Context: гайдбуки пишутся не только по-английски, а сервис работает на CPU внутри того же контейнера, что и API.
Decision: `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` через `fastembed` (полное имя обязательно — реестр `fastembed` отклоняет короткое), вектор 384 измерения, окно 120 токенов при пределе входа 128.
