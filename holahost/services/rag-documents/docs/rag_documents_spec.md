...
ошибки — Этап 7 (наследуется из `lead-capture` §10.8).
...
- Пустое имя или имя длиннее `MAX_DOCUMENT_NAME_LENGTH` → `400 ERR_INVALID_PAYLOAD` с указанием поля.
...
- Запрос длиннее `MAX_QUERY_LENGTH` → `400 ERR_INVALID_PAYLOAD`.
- Пустой запрос → `400 ERR_INVALID_PAYLOAD`.
...
Clean architecture, слои и правила импортов — как в `lead-capture` (`import-linter`, контракт
`layers = ["interface", "infrastructure", "application", "domain"]`, `config` — leaf-исключение,
`scripts/` вне контракта). Отличие: адаптер интерфейса — FastAPI, а не Lambda.
...
Lightweight DDD, идентично `lead-capture`: `domain/entities/` с фабричными методами `create()` и
...
| Чанкинг | стек `langchain` переиспользуется из `lead-capture` целиком: здесь `langchain-text-splitters`, чат-модели того же стека — в `llm-client` |
...
Одинаков для всех роутов, кроме `GET /health`:

1. `interface/http/middleware.request_id` — берёт `X-Request-ID` или отсутствие такового фиксирует как факт; кладёт в контекст логирования.
...
3. `RateLimiter.check(client_id, subject, bucket)` — `429 + Retry-After` при превышении; bucket = `"ingest"` для `POST`/`PUT`, `"read"` для остальных.
4. `interface/http/schemas` — разбор и валидация тела Pydantic-моделью; `400` при несоответствии.
5. Вызов use case'а.
...
Context: `lead-capture` держал embeddings в `bytea` и считал cosine numpy в процессе — это было оправдано stateless-Lambda с одним гайдбуком на сессию; здесь сервис always-on и документов много.
...
Context: в `lead-capture` действовала жёсткая связка «email ↔ один guidebook»; на платформе владельцем становится субъект токена, и обсуждался домен-агностичный контейнер `Collection`.
...
Decision: эндпоинты — синхронные `def`, FastAPI исполняет их в threadpool; SQLAlchemy Core и `psycopg` — синхронные, инфраструктурный слой `lead-capture` переиспользуется без переписывания.
...
Consequences: инфраструктурный слой переиспользован без переписывания, класса ошибок «блокирующий вызов в корутине» не существует. Взамен конкурентность ограничена размером пула потоков, и при её исчерпании запросы встают в очередь, а не отклоняются явно.
...
**A-15. Модель эмбеддингов — многоязычная MiniLM-L12, 384 измерения** ⚠ Пропуск: выбор унаследован из lead-capture, отдельным черновиком не оформлялся.
