...
| `accepted` | алиас/`model_id` неизвестны либо провайдер выключен | `rejected_model` | 400 `ERR_UNKNOWN_MODEL` |
| `resolved` | вход длиннее `max_context` модели | `rejected_context` | 422 `ERR_CONTEXT_OVERFLOW` |
| `resolved` | бюджет вызывателя или провайдера исчерпан, политика `reject` | `rejected_budget` | 429 `ERR_BUDGET_EXHAUSTED` + время сброса |
| `resolved` | бюджет исчерпан, политика `downgrade`, удешевлённая модель доступна | `budget_ok` | — (фактическая модель вернётся в ответе) |
| `downgraded` | удешевлённой модели нет либо её бюджет тоже исчерпан | `rejected_budget` | 429 `ERR_BUDGET_EXHAUSTED` + время сброса |
...
| `retrying` | попытки и цепочка исчерпаны | `failed_upstream` | 502 `ERR_UPSTREAM_LLM` (retryable) |
...
Формат тела ошибки и полный список `ERR_*` — Этап 7.
...
- Запрос больше `MAX_INPUT_BYTES` → `413 ERR_PAYLOAD_TOO_LARGE`.
- Пустой список `messages` → `400 ERR_INVALID_PAYLOAD`.
...
- Запрос принимает ключ идемпотентности. Повтор с тем же ключом в пределах `IDEMPOTENCY_KEY_TTL`, пока первый в работе либо уже завершён, → `409 ERR_DUPLICATE_REQUEST`; второй вызов провайдера не выполняется и второй записи расхода не появляется.
...
- Неизвестный алиас или `model_id` → `400 ERR_UNKNOWN_MODEL`; в `details` — список доступных алиасов.
- Алиас, указывающий на выключенного провайдера или на удалённую модель, → `400 ERR_UNKNOWN_MODEL`.
...
- Исчерпание цепочки → `502 ERR_UPSTREAM_LLM` с признаком «повторяемо».
...
- Исчерпан потолок `BUDGET_CAP_PER_CLIENT` или `BUDGET_CAP_PER_PROVIDER` при политике `reject` → `429 ERR_BUDGET_EXHAUSTED`; в `details` — какой потолок исчерпан и время начала следующего окна.
...
- Если `DOWNGRADE_MODEL` не задана либо её собственный бюджет тоже исчерпан → `429 ERR_BUDGET_EXHAUSTED`.
...
**AC:**
- Оценка входа больше `max_context` выбранной модели → `422 ERR_CONTEXT_OVERFLOW`; в `details` — лимит модели и оценка запроса.
...
**AC:**
- Подключена общая библиотека `holahost-auth` (рамочная спека, «Переиспользуемые общие сущности») на все роуты, кроме `GET /health`.
...
**AC:**
- Лимит применяется после `holahost-auth` и до обработчика; ключ — `client_id`+`sub`.
...
**AC:**
- Тело ошибки — `{ error: { code, message, details } }`; `code` из таксономии `ERR_*`.
- Вендорские коды и сообщения провайдера наружу не пробрасываются; они пишутся в лог.
- Каждому `ERR_*` сопоставлена retry-семантика; `ERR_UPSTREAM_LLM` помечен повторяемым, `ERR_CONTEXT_OVERFLOW` и `ERR_UNKNOWN_MODEL` — нет.
...
backend/
  app/
    ...
    interface/
      http/                 # FastAPI: роутер, схемы, middleware, error handler
...
| Клиент провайдеров | стек `langchain`, переиспользуемый из `lead-capture`: чат-модели провайдеров (`langchain_anthropic` и аналоги) за портом |
...
| `MAX_OUTPUT_TOKENS` | 1000 | из `lead-capture` §10.2; выше — риск не уложиться в `PROVIDER_TIMEOUT` |
| `BUDGET_WINDOW` | сутки, сброс 00:00 UTC, lazy при первом запросе нового окна | механика `sample_budget` из `lead-capture` |
...
| Аспект | Решение |
|---|---|
| Аутентификация | JWT оффлайн через `holahost-auth` на всех роутах, кроме `GET /health` |
...
| Авторизация | `Authorization: Bearer <jwt>` на всех роутах, кроме `GET /health`; ключей провайдеров контракт не принимает |
...
| Схема | OpenAPI сервиса версионируется в его репозитории |
...
### 7.4 Таксономия ошибок

| Код | HTTP | Когда | `details` | Повторять? |
|---|---|---|---|---|
| `ERR_INVALID_PAYLOAD` | 400 | пустой `messages`, неизвестная роль, `max_tokens` ≤ 0 | `field` | нет |
| `ERR_UNKNOWN_MODEL` | 400 | алиас или модель не резолвятся, провайдер выключен | `requested`, `available_aliases` | нет |
| `ERR_PAYLOAD_TOO_LARGE` | 413 | тело больше `MAX_INPUT_BYTES` | `limit`, `actual` | нет |
| `ERR_CONTEXT_OVERFLOW` | 422 | оценка входа больше `max_context` модели | `max_context`, `estimated` | нет, пока вход не уменьшен |
| `ERR_REQUEST_TOO_SLOW_FOR_SYNC` | 422 | pre-flight: оценка времени не помещается в бюджет (§3.7.1) | `max_tokens_allowed`, `budget_seconds` | нет, пока `max_tokens` не уменьшен |
| `ERR_DUPLICATE_REQUEST` | 409 | `Idempotency-Key` уже в работе или уже завершён | `state` (`in_flight`/`completed`) | нет |
| `ERR_BUDGET_EXHAUSTED` | 429 | исчерпан потолок при политике `reject`; `Retry-After` обязателен | `scope` (`client`/`provider`), `resets_at` | да, после `resets_at` |
| `ERR_RATE_LIMIT` | 429 | превышен per-caller лимит; `Retry-After` обязателен | `retry_after_seconds` | да, после паузы |
| `ERR_UPSTREAM_LLM` | 502 | попытки и цепочка провайдеров исчерпаны | `attempts`, `upstream_status` | да |
| `ERR_INTERNAL` | 500 | необработанная ошибка | — | да, однократно |

Два разных отказа отвечают `429`, и различаются они кодом, а не статусом: `ERR_RATE_LIMIT` — «слишком
часто, подожди секунды», `ERR_BUDGET_EXHAUSTED` — «деньги на сегодня кончились, подожди до сброса
окна». Склеивать их в один код нельзя: реакции у вызывателя разные.
...
class RateLimiter(Protocol):
    def check(self, client_id: str, subject: str) -> None: ...
    # raises: RateLimitExceededError (несёт retry_after)
    # concurrency: счётчик в памяти процесса; проверка и инкремент атомарны относительно потоков пула
```

`ProvidersRepo` читает git-конфиг, загруженный при старте; `IdempotencyStore` и `RateLimiter`
реализованы в памяти процесса; `BudgetRepo` и `UsageRepo` ходят в Postgres.
...
1. `interface/http/middleware.request_id` — `X-Request-ID` в контекст логирования.
2. `holahost_auth.middleware.authenticate(request) -> TokenContext` — `401` при неудаче.
3. `RateLimiter.check(client_id, subject)` — `429 ERR_RATE_LIMIT` при превышении.
4. `interface/http/schemas.GenerateRequest` — разбор тела и заголовка `Idempotency-Key`; `400`/`413`.
5. `GenerateUseCase.execute(cmd)`.
...
|---|---|---|
| `DuplicateRequestError` | use case | транслирует в `409 ERR_DUPLICATE_REQUEST` с состоянием из исключения |
| `TransientProviderError` | use case, шаг 1.8 | повтор с backoff, затем следующий провайдер цепочки; исчерпание — `502 ERR_UPSTREAM_LLM` |
...
| `RateLimitExceededError` | мидлварь, до use case | `429` + `Retry-After` из исключения |
...
### 8.4 Технические метрики

Как и у прочих сервисов платформы, метрика — структурное лог-событие с фиксированными полями;
агрегация задаётся на стороне сбора логов (Этап 11).

```
generation_completed { request_id, client_id, subject, outcome,
                       requested_model, provider, model,
                       input_tokens, output_tokens,
                       duration_ms, provider_ms, attempts,
                       downgraded, failed_over, preflight_rejected }
```

| Метрика | Из какого поля | Зачем |
|---|---|---|
| p50/p95/p99 длительности | `duration_ms` | контроль `GENERATION_P95_BUDGET` |
| Доля времени провайдера | `provider_ms / duration_ms` | показывает, сколько тратит сам сервис поверх вендора |
| Расход токенов по вызывателю, провайдеру и модели | `input_tokens`, `output_tokens` | основа отчётности и прогноза бюджета |
| Утилизация бюджета | расход за окно к `BUDGET_CAP_*` | предупреждение до того, как начнутся отказы |
| Частота `downgrade` и `failover` | `downgraded`, `failed_over` | скрытая деградация качества: рост означает, что вызыватели давно получают не ту модель, которую просят |
| Распределение `attempts` | `attempts` | здоровье провайдера; устойчивый рост — повод пересмотреть `RETRY_*` |
| Частота pre-flight-отказов | `preflight_rejected` | если она заметна, синхронный режим перестал покрывать нагрузку — триггер Р-3 |
| Частота отказов по бюджету и по лимиту | `outcome` | различаются кодом, реакции у вызывателя разные |
| Доля таймаутов провайдера | `outcome = ERR_UPSTREAM_LLM` с `attempts` на максимуме | оценивает объём расхода, прошедшего мимо журнала (B-12) |

Тексты `system`, `messages` и ответа, а также ключи провайдеров в события не попадают (US-L13).
...
| **Алармы** (`infra/envs/<env>`) | сверх умолчания — на долю `ERR_UPSTREAM_LLM`, на утилизацию суточного бюджета и на долю `downgraded`: последняя ловит тихую деградацию качества, которую иначе никто не заметит (B-4) |
| **Runbook, раздел «Проверить»** | профильная операция — генерация на дешёвой модели с коротким `max_tokens`; в логах смотреть `generation_completed`. Проверка обязана быть дешёвой: она тратит настоящие деньги |
...
- `L-06` Application-исключения и их соответствие кодам `ERR_*`
...
- `L-12` In-memory idempotency store с TTL и rate limiter
- `L-13` Структурное логирование — allowlist полей, `X-Request-ID`, событие `generation_completed`; промпты и ключи в логи не попадают

### Backend — Interface

- `L-14` FastAPI-приложение, роутер, схемы запроса и ответа, заголовок `Idempotency-Key`
- `L-15` Мидлвари и обработчик ошибок — авторизация, rate limit, envelope, два разных `429`
- `L-16` `GET /health` без обращения к провайдерам
...
Context: у `lead-capture` промпт «ответ гостю» жил рядом с LLM-адаптером; на платформе потребителей LLM будет несколько, со своими сценариями.
...
Context: в `lead-capture` ключ приносил конечный пользователь (`X-Api-Key`) — это было требованием публичного демо; здесь вызыватели — сервисы платформы.
...
Decision: исчерпав ретраи у текущего провайдера, сервис переключается на следующего в заданной конфигом цепочке; исчерпание цепочки → `502 ERR_UPSTREAM_LLM`; фактический провайдер возвращается в ответе.
...
**B-13. Метрики — структурные лог-события, без отдельной системы метрик** ⚠ Пропуск: решение принято в §8.4, черновиком не оформлялось.
...
**B-14. `system` — отдельное поле контракта, а не сообщение с ролью** ⚠ Пропуск: решение принято в §7.2, черновиком не оформлялось.
