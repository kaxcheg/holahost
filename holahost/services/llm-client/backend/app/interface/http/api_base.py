...
The platform makes `llm-client` a single identity with three derived uses: the container name
on the `backbone` network, the ECR repository name, and this path segment —
`API_BASE_URL = /api/llm-client`, a bare path with no domain, because the gateway routes by path
