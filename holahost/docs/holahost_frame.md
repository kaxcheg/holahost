...
| staging / prod | AWS Secrets Manager, ресурсы создаёт TF-root сервиса | контейнер на старте по instance role (у роли — SM read) | замена значения в SM + рестарт контейнера, выката образа не требуется |
...
1. **Развернуть с нуля** — от пустого окружения до отвечающего `GET /health`: применение TF-root'ов,
   заполнение секретов, первый выкат, миграции.
2. **Проверить** — smoke `GET /health` плюс проверка профильной операции сервиса; где смотреть логи.
...
| `<svc>-deploy-staging` | push в `release/v*` | `terraform apply` → `docker build` и push в ECR (`git-<sha>` + `release-v<version>`), запоминание digest → SSM Run Command: `alembic upgrade head`, затем `docker compose up -d` → smoke |
| `<svc>-promote-prod` | push тега `<svc>/v*` | `terraform apply` → resolve digest по тегу `release-v<version>` **без пересборки** → SSM Run Command: миграции, затем выкат тем же digest → smoke → gate `environment: prod` с ручным approve |
