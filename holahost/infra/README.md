# Holahost — Environment Runbook

Operational runbook for provisioning and operating the **Holahost platform** (domain `hola.host`,
frontend hosting, CDN/edge, ECR registry, GitHub settings) and its services on AWS (spec §12.4). The
lead-capture service is deployed on top of the platform. Covers the one-time bootstrap (AWS account,
Terraform backend, Neon), every Terraform root, the manual deploy · smoke · rollback · access
procedures, and the Resend email setup.
...
**Topology (Terraform roots, monorepo).** Platform infra is `holahost/infra/`; the lead-capture service
is an env-agnostic **module** (`holahost/services/lead-capture/infra/`) that the app's env roots
instantiate. Roots under `holahost/infra/envs/`:

| Root | Provisions | State bucket |
|---|---|---|
...
**Configuration.** Two static configs: [`holahost/infra/config.yaml`](config.yaml) (platform — domain,
frontend bucket, subdomains, ECR repos, published-asset paths/keys) and
`holahost/services/lead-capture/infra/config.yaml` (service intrinsic — secret keys, per-env Lambda
sizing). Per-env backend runtime config is the committed `holahost/services/lead-capture/envs/<env>.env`.
Full app-vs-infra settings map: spec **§10.9**
([`lead_capture_spec.md`](../services/lead-capture/docs/lead_capture_spec.md)).
...
for ENV in common codebase staging prod; do
...
1. **The dev backend config** `holahost/services/lead-capture/envs/dev.env` is **committed** (non-secret) —
   no copy step. It targets the compose stack
   (`DATABASE_URL=postgresql://holahost:holahost@postgres:5432/holahost`; `IP_HASH_SALT` + `RESEND_API_KEY`
   are dev-local dummies — dev email goes to Mailpit). The one real secret, `SAMPLE_SERVER_API_KEY`, is
   NOT stored in the file — `make dev-up` prompts for it (step 2).
...
3. **Frontend dev-server** runs separately on the host (HTTP, hot reload):
   ```bash
   npm run dev --prefix holahost/frontend
...
GitHub repository settings (`holahost/infra/envs/codebase/` → `github_repo` module): repo options, branch
protection for `main` / `develop`, and the GitHub Environments `staging` / `prod` (§13.7). A separate
root/state (`holahost-tfstate-codebase`) — the repository is a single instance tied to neither AWS env.
...
2. **Init + import the pre-existing repo** (one-time — Terraform manages it, never creates it):
   ```bash
   cd holahost/infra/envs/codebase
   terraform init
...
(cd holahost/services/lead-capture/backend && poetry run alembic upgrade head)
...
npm run build:staging --prefix holahost/frontend
aws s3 sync holahost/frontend/dist/ "s3://holahost-frontend/releases/$GIT_SHA/"
...
- **DB**: `(cd holahost/services/lead-capture/backend && poetry run alembic downgrade -1)` for a reversible migration; for a
...
`hola.host`, frontend build `npm run build --prefix holahost/frontend` (prod mode). The frontend bucket and the
...
2. **Copy the records into `infra/config.yaml` → `email_dns_records`** (keyed by a free-form label;
   ...
   ```
3. **Apply the `shared` root** (owns the zone): `cd holahost/infra/envs/common && terraform apply`.
