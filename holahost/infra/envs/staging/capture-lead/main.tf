# Holahost places the lead-capture service into STAGING. Reads the platform singletons (frontend bucket
# ARN, ECR repo URL) from `common` state, assembles the env config (../staging.env + the frontend
# magic-link contract), and instantiates the env-agnostic service module. Applyable on its own
# (independent service deploy) once `common` exists.
locals {
  ...
  # Per-env sizing the service DECLARES (env chosen here by the app).
  ...
  # Frontend-owned magic-link contract (holahost/frontend/.env).
  fe_env_lines = [
