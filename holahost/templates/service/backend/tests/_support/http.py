...
`create_edge_app` does this for the real application, from the same two declarations. A
test that builds a minimal app — routes only, or one middleware under examination — still
needs the handlers, or every published error comes back `500` and the test is measuring the
wrong thing.
