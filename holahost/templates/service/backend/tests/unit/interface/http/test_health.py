"""Unit tests for GET <API_BASE_URL>/health."""
...
# Mounted the same way `create_app()` mounts it, under the service's own base path — so
# these tests exercise the real, gateway-reachable URL rather than a bare `/health` no
...
class TestHealth:
    def test_ok_when_the_dependencies_are_ready(self) -> None:
        app = _build_app()
    ...
    def test_503_when_a_dependency_is_unavailable(self) -> None:
        app = _build_app()
