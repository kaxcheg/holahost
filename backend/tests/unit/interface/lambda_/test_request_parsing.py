...
def test_parse_resolve_magic_link_from_header() -> None:
    evt = _evt(method="GET", path="/api/magic-link/resolve", headers={"x-magic-link": "MTOK"})
...
def test_capture_missing_required_field_raises_invalid_json() -> None:
    ...
    with pytest.raises(InvalidPayloadError) as exc:
        rp.parse_capture_lead(_evt(path="/api/leads/capture", body=body), make_settings())
...
def test_capture_honeypot_optional_defaults_empty() -> None:
    ...
    cmd = rp.parse_capture_lead(_evt(path="/api/leads/capture", body=body), make_settings())
    assert cmd.honeypot == ""
